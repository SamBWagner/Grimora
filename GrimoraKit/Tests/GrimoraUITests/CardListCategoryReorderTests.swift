...
final class CardListCategoryReorderTests: XCTestCase {
  func testDestinationPositionMapsRepresentativeMoves() {
    ...
    XCTAssertEqual(CardListCategoryReorder.destinationPosition(forMoveFrom: IndexSet(integer: 1), to: 0), 0)
    XCTAssertEqual(CardListCategoryReorder.destinationPosition(forMoveFrom: IndexSet(integer: 0), to: 3), 2)
    XCTAssertEqual(CardListCategoryReorder.destinationPosition(forMoveFrom: IndexSet(integer: 2), to: 0), 0)
    XCTAssertEqual(CardListCategoryReorder.destinationPosition(forMoveFrom: IndexSet(integer: 0), to: 2), 1)
    XCTAssertEqual(CardListCategoryReorder.destinationPosition(forMoveFrom: IndexSet(integer: 2), to: 2), 2)
  }

  func testDestinationPositionReturnsNilForEmptySource() {
    XCTAssertNil(CardListCategoryReorder.destinationPosition(forMoveFrom: IndexSet(), to: 1))
  }
    ...
    for count in 1...6 {
      ...
      for from in 0..<count {
        for offset in 0...count where offset != from && offset != from + 1 {
          ...
          let destination = CardListCategoryReorder.destinationPosition(
