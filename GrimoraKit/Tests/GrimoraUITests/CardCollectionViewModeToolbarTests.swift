...
/// from `CardCollectionViewMode`'s display metadata. These tests lock that metadata so
...
final class CardCollectionViewModeToolbarTests: XCTestCase {
    func testEveryModeHasNonEmptyTitleAndSymbol() {
        for mode in CardCollectionViewMode.allCases {
            XCTAssertFalse(
    ...
    func testTitlesAndSymbolsAreUniqueAcrossModes() {
        let titles = CardCollectionViewMode.allCases.map(\.toolbarTitle)
        let symbols = CardCollectionViewMode.allCases.map(\.systemImage)
    ...
    func testGridAndListUseExpectedLabels() {
        XCTAssertEqual(CardCollectionViewMode.grid.toolbarTitle, "Gallery")
        XCTAssertEqual(CardCollectionViewMode.list.toolbarTitle, "List")
    }
}
