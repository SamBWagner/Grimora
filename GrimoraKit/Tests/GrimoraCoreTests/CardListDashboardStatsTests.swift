...
final class CardListDashboardStatsTests: XCTestCase {
    func testDashboardStatsAreQuantityWeightedAndSkipUnavailablePriceData() {
        ...
        let statsWithLands = CardListDashboardStats.make(entries: entries, includeLandsInTypes: true)
    ...
    private func colorQuantity(_ bucket: CardListColorBucket, in stats: CardListDashboardStats) -> Int? {
        stats.colorDistribution.first { $0.bucket == bucket }?.quantity
    }

    private func colorPercentage(_ bucket: CardListColorBucket, in stats: CardListDashboardStats) -> Double {
        stats.colorDistribution.first { $0.bucket == bucket }?.percentage ?? 0
    }

    private func typePercentage(_ name: String, in stats: CardListDashboardStats) -> Double {
        stats.topTypes.first { $0.name == name }?.percentage ?? 0
    }

    private func entry(card: CardRecord?, quantity: Int, position: Int) -> CardListEntryRecord {
        CardListEntryRecord(
            id: "entry-\(position)",
            listID: "list",
