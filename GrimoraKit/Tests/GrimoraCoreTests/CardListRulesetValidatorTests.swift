...
final class CardListRulesetValidatorTests: XCTestCase {
    func testConstructedRulesetWarningsCoverDeckSizeSideboardCopiesAndLegality() {
        ...
        let warningIDs = Set(CardListRulesetValidator.warnings(for: list, entries: entries).map(\.id))
    ...
    private func list(ruleset: CardListRuleset) -> CardListRecord {
        CardListRecord(
            id: "list",
            name: "List",
            ruleset: ruleset,
