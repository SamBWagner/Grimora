...
struct CardCollectionsSidebarContent: View {
            ...
            if lists.isEmpty {
                ...
                ForEach(Array(lists.enumerated()), id: \.element.id) { index, list in
                    CardCollectionSidebarRow(
                        ...
                        onSelect: { model.selectCardCollection(id: list.id) },
                        onRename: { onRenameList(list) },
                        onTogglePin: {
                            model.setCardCollectionPinned(id: list.id, isPinned: !list.isPinned)
                        },
                        onDelete: { model.deleteCardCollection(id: list.id) }
                    )
                }
            }
        }
...
struct CardCollectionSidebarRow: View {
    @Environment(GrimoraAppModel.self) private var model

    var list: CardCollectionRecord
    var listIndex: Int
    var listCount: Int
    var sectionLists: [CardCollectionRecord]
    var isPinnedSection: Bool
    var palette: GrimoraPalette
    @Binding var draggedListID: CardCollectionRecord.ID?
    var isSelected: Bool
    ...
    var body: some View {
        ...
        HStack(spacing: 8) {
            if let systemSymbol = model.systemSymbol(for: list) {
                ...
                .accessibilityHint("Unpins this collection")
    ...
    private var listActionMenuContent: some View {
        ...
        if !isFirst || !isLast {
            ...
            if !isFirst {
                Button {
                    model.moveCardCollection(id: list.id, toPosition: 0, isPinned: list.isPinned)
                } label: {
                    Text("Move to Top")
                }
                ...
                Button {
                    model.moveCardCollection(id: list.id, by: -1)
                } label: {
                    Text("Move Up")
                }
            ...
            if !isLast {
                ...
                Button {
                    model.moveCardCollection(id: list.id, toPosition: listCount - 1, isPinned: list.isPinned)
                } label: {
                    Text("Move to Bottom")
                }
...
struct CardCollectionDropDelegate: DropDelegate {
    ...
    var targetList: CardCollectionRecord?
    ...
    static func isListDragToken(_ value: String) -> Bool {
        CardCollectionDragToken.isListDragToken(value)
    }
    ...
    private func moveDraggedListIfNeeded() {
        ...
        model.moveCardCollection(id: draggedListID, toPosition: targetIndex, isPinned: targetIsPinned)
    }
}
...
extension View {
    ...
    func draggableList(
        if isEnabled: Bool,
        draggedListID: Binding<CardCollectionRecord.ID?>,
        listID: CardCollectionRecord.ID
    ) -> some View {
        if isEnabled {
            onDrag {
                draggedListID.wrappedValue = listID
                return CardCollectionDropDelegate.dragProvider(for: listID)
            }
        } else {
            self
        }
