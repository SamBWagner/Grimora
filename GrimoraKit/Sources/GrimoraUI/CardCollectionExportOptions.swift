...
struct CardCollectionExportOptions: View {
    @Binding var configuration: CardCollectionExportConfiguration
    ...
    var body: some View {
        ...
        case .text:
            Section("Export options") {
                ...
                Picker("Section header", selection: $configuration.textSectionHeader) {
                    ForEach(CardCollectionExportTextSectionHeader.allCases) { sectionHeader in
        ...
        case .csv:
            Section("Export options") {
                ...
                HStack {
                    Button("Check all") {
                        configuration.csvColumns = CardCollectionExportCSVColumn.allCases
                    }
                ...
                ForEach(CardCollectionExportCSVColumn.allCases) { column in
        ...
        case .deckRegistrationPDF:
            Section("Deck registration") {
                ...
                Picker("Sort by", selection: $configuration.deckRegistrationSortMode) {
                    ForEach(CardCollectionDeckRegistrationSortMode.allCases) { sortMode in
        ...
        case .edhrecArticle:
            Section("Export options") {
                Text("EDHREC article export includes category headings when this collection has categories.")
                    .foregroundStyle(.secondary)
            }
        case .grimoraArchive:
            Section("Export options") {
    ...
    private func csvColumnBinding(_ column: CardCollectionExportCSVColumn) -> Binding<Bool> {
        Binding {
            configuration.csvColumns.contains(column)
        } set: { isSelected in
            if isSelected {
                configuration.csvColumns = CardCollectionExportCSVColumn.allCases.filter { candidate in
