...
struct CardCollectionTextRowView: View {
    ...
    var body: some View {
        HStack(alignment: .center, spacing: 10) {
            CardCollectionTextRowLeadingIcon(
    ...
    private var moveZoneMenu: some View {
        let zones = model.selectedCollection?.ruleset.allowedZones ?? CardCollectionRuleset.none.allowedZones
        if zones.count > 1 {
            Menu {
...
enum CardCollectionTextRowColorIdentityStyle: Equatable {
    case colorless
    case mono(String)
    ...
    // Compiled once, not per row: `String.range(of:options:.regularExpression)` builds a fresh
    // NSRegularExpression on every call, which is wasteful for a check that runs for every visible row.
    private static let devoidRegex = try! NSRegularExpression(pattern: #"\bdevoid\b"#, options: [.caseInsensitive])

    private static func mentionsDevoid(_ text: String) -> Bool {
        // Cheap substring gate first: the overwhelming majority of cards never mention "devoid", so we
        // skip the regex engine entirely for them and only pay for word-boundary matching when needed.
        guard text.range(of: "devoid", options: .caseInsensitive) != nil else {
            return false
        }
        let range = NSRange(text.startIndex..<text.endIndex, in: text)
        return devoidRegex.firstMatch(in: text, range: range) != nil
    }
    ...
    private static func isDevoid(_ card: CardRecord) -> Bool {
        ...
        if mentionsDevoid(card.oracleText) {
            return true
        }

        return card.faces.contains { mentionsDevoid($0.oracleText) }
    }
}

struct CardCollectionTextEmptyCategoryRow: View {
    var palette: GrimoraPalette
