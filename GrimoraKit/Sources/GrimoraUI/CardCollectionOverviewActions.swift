...
/// actions (CardCollectionBrowserRowActions): the protected Favourites list isn't
/// pinnable, renamable, or deletable.
struct CardCollectionOverviewActions: View {
    ...
    var body: some View {
        if !model.isProtectedFavouritesList(item.list) {
            Button {
                model.setCardCollectionPinned(id: item.list.id, isPinned: !item.list.isPinned)
            } label: {
            ...
            Button(role: .destructive) {
                model.deleteCardCollection(id: item.list.id)
            } label: {
