...
struct CardCollectionNewCategoryPromptModifier: ViewModifier {
...
extension View {
    func cardCollectionNewCategoryPrompt(
        isPresented: Binding<Bool>,
        onCreate: @escaping (String) -> Void
    ) -> some View {
        modifier(CardCollectionNewCategoryPromptModifier(isPresented: isPresented, onCreate: onCreate))
    }
}
...
struct CardCollectionCategorySectionView: View {
    ...
    var body: some View {
        HStack(alignment: .center, spacing: 10) {
            ...
            if let category = section.category {
                CardCollectionCategoryMovementControls(
                    category: category,
...
struct CardCollectionCategoryReorderView: View {
    @Environment(GrimoraAppModel.self) private var model
    @State private var draggedCategoryID: CardCollectionCategoryRecord.ID?
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            ForEach(Array(categories.enumerated()), id: \.element.id) { index, category in
                CardCollectionCategoryReorderRow(
                    category: category,
                    categoryIndex: index,
        ...
        .sheet(isPresented: $isPresentingReorderOverlay) {
            CardCollectionCategoryReorderSheet(palette: palette, model: model)
        }
        #endif
    }
...
struct CardCollectionCategoryReorderRow: View {
    var category: CardCollectionCategoryRecord
    var categoryIndex: Int
    var categoryCount: Int
    var palette: GrimoraPalette
    @Binding var draggedCategoryID: CardCollectionCategoryRecord.ID?
    ...
    var body: some View {
        ...
        .onDrop(
            of: [.plainText],
            delegate: CardCollectionCategoryDropDelegate(
    ...
    private static func dragToken(for categoryID: CardCollectionCategoryRecord.ID) -> String {
        CardCollectionCategoryDropDelegate.dragTokenPrefix + categoryID
    }
}

struct CardCollectionCategoryDropDelegate: DropDelegate {
    static let dragTokenPrefix = "grimora-category:"

    var targetCategory: CardCollectionCategoryRecord
    var categories: [CardCollectionCategoryRecord]
    @Binding var draggedCategoryID: CardCollectionCategoryRecord.ID?
    var model: GrimoraAppModel
    ...
    private func moveDraggedCategoryIfNeeded() {
        ...
        model.moveCardCollectionCategory(id: draggedCategoryID, toPosition: toIndex)
    }
}
...
/// (`CardCollectionCategoryReorderRow` + `CardCollectionCategoryDropDelegate`).
struct CardCollectionCategoryReorderSheet: View {
    @Environment(\.dismiss) private var dismiss
    var model: GrimoraAppModel
    @State private var workingCategories: [CardCollectionCategoryRecord]
    ...
    init(palette: GrimoraPalette, model: GrimoraAppModel) {
        ...
        self.model = model
        _workingCategories = State(initialValue: model.selectedCollectionCategories)
    }
    ...
    var body: some View {
        NavigationStack {
            ...
            .onChange(of: model.selectedCollectionCategories) { _, newValue in
    ...
    private func row(for category: CardCollectionCategoryRecord) -> some View {
        HStack(alignment: .center, spacing: 10) {
            Image(systemName: "folder")
                .foregroundStyle(palette.secondaryText.color)
                .frame(width: 18)
    ...
    private func move(from source: IndexSet, to offset: Int) {
        guard let sourceIndex = source.first,
              ...
              let destination = CardCollectionCategoryReorder.destinationPosition(forMoveFrom: source, to: offset)
        else {
            return
        }
    ...
    private func entryCountText(for category: CardCollectionCategoryRecord) -> String {
        let count = category.entryCount
        let noun = count == 1 ? "card" : "cards"
        return "\(count.formatted()) \(noun)"
    }
...
enum CardCollectionCategoryReorder {
    /// Maps SwiftUI's `.onMove` destination offset (expressed in pre-removal indices) to the
    /// post-removal insertion index expected by `GrimoraAppModel.moveCardCollectionCategory(id:toPosition:)`,
...
struct CardCollectionCategoryMovementControls: View {
    @Environment(GrimoraAppModel.self) private var model
    ...
    var body: some View {
        HStack(spacing: 6) {
            if showsMoveToTop {
                Button {
                    model.moveCardCollectionCategory(id: category.id, toPosition: 0)
                } label: {
            ...
            Button {
                model.moveCardCollectionCategory(id: category.id, by: -1)
            } label: {
    ...
    private var categoryActionsMenu: some View {
        Menu {
            ...
            Button {
                model.moveCardCollectionCategory(id: category.id, toPosition: categoryCount - 1)
            } label: {
                Text("Move to Bottom")
            }
            .disabled(isLast)
            ...
            Button(role: .destructive) {
                model.deleteCardCollectionCategory(id: category.id)
            } label: {
                Text("Delete")
            }
...
struct CardCollectionMoveCategoryMenu: View {
    @State private var moveFeedbackTrigger = 0
    ...
    var body: some View {
        if !categories.isEmpty || entry.categoryID != nil {
            HStack(spacing: 5) {
                ...
                .id("\(entry.id)-category-menu-\(entry.categoryID ?? CardCollectionEntrySection.uncategorizedID)")
...
struct CardCollectionMoveZoneMenu: View {
    @State private var moveFeedbackTrigger = 0
...
struct MissingCardCollectionEntryView: View {
